...
export type ApprovalRequestProps = {
  ...
  onDeny?: () => void;
  /**
   * True while a decision is in flight. The buttons stay visible and go
   * inert rather than disappearing — a card that empties itself the instant
   * you click reads as "did that work?", and the answer arrives over the
   * event stream a moment later.
   */
  busy?: boolean;
  /** Small note beside the buttons — e.g. how long it has been waiting. */
        ...
        <div className="min-w-0 flex-1">
          ...
          {status === "pending" ? (
            <div className="mt-3 flex items-center gap-2">
              ...
              {note && <span className="ml-auto text-[11px] text-zinc-500 dark:text-zinc-400">{note}</span>}
            </div>
          ) : (
            <p
