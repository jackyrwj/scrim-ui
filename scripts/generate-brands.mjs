 ...
 *
 * Brand marks are trademarks of their owners; we render them monochrome, at
 * small sizes, purely to identify the product being linked or discussed.
 * Simple Icons ships the official paths under CC0.
 *
...
for (const slug of SLUGS) {
  ...
  entries.push([slug, { title: icon.title, viewBox: "0 0 24 24", body: `<path d="${icon.path}"/>` }]);
}
for (const [key, data] of Object.entries(MANUAL)) entries.push([key, { ...data }]);
 ...
 * set of hand-added marks; each is inlined with fill=currentColor so it picks
 * up the theme text color. Logos are trademarks of their respective owners and
 * are used only to identify the product being named. Names with no mark here
 * fall back to a two-letter avatar in <BrandIcon>.
 */
export type BrandData = { title: string; viewBox: string; body: string };

export const brandData: Record<string, BrandData> = {
${entries.map(([key, d]) => `  ${q(key)}: { title: ${q(d.title)}, viewBox: ${q(d.viewBox)}, body: ${q(d.body)} },`).join("\n")}
};
